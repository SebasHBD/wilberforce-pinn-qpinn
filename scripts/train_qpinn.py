"""Entrena el Q-PINN hibrido siguiendo el curriculum de 3 etapas.

Uso:
    python scripts/train_qpinn.py --semilla 1
...
from wilberforce.checkpoints import guardar_checkpoint_qpinn
...
for t_max, n_puntos, epocas_adam, epocas_lbfgs in ETAPAS:
    ...
    loss_ic, loss_phy, loss_lambda, grad_ic, grad_phy, ntk_eigvals, ntk_epochs = qpinn.train(
    ...
    base = os.path.join(RAIZ, "results", "figures", f"qpinn_semilla{SEED}_t{int(t_max)}")
    ...
    graficar_prediccion(qpinn, tau_pred, ruta_salida=base + "_prediccion.png")

    guardar_checkpoint_qpinn(qpinn, int(t_max), SEED)